...
try {
    ...
    $stmt = $conexion->prepare("SELECT NombreCliente FROM Cliente WHERE idCliente = ? ");
    ...
    if ($resultado->num_rows > 0) {
        ...
        if($row['NombreCliente'] === null || $row['NombreCliente'] == ""){
            echo json_encode(["Nombre" => "Usuario Sin Nombre"]);
        }else{
            echo json_encode(["Nombre" => $row['NombreCliente']]);
        }
        exit();
    } else {
